...
`T` light/dark theme (persisted), `1-6` jump to act, `R` reset, `H` show or
hide the key bar (hidden by default). The URL hash tracks the current step,
so a refresh restores position. Fully offline: Three.js and the Space Grotesk
